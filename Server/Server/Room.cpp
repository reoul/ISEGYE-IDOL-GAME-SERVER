...
unsigned Room::ProgressThread(void* pArguments)
{
	...
	// 기본 템 지급
	{
		...
		for (Client* client : room.mClients)
		{
			...
			client->SetNormalItemTicketCount(DEFAULT_ITEM_TICKET_COUNT);
			client->SetAdvancedItemTicketCount(DEFAULT_ITEM_TICKET_COUNT);
			client->SetTopItemTicketCount(DEFAULT_ITEM_TICKET_COUNT);
			client->SetSupremeItemTicketCount(DEFAULT_ITEM_TICKET_COUNT);
...
bool Room::ReadyStage(Room& room, bool isNextStageBattle)
{
...

	{
		...
		{
			sc_SetReadyTimePacket packet(readyTime);
			packet.Write(memoryStream);
		}
...
bool Room::BattleStage(Room& room)
{
		...
		{
			{
				...
				for (int k = 0; k < avatarCount; k += 2)
				{
					...
					{
						avatars[k].ToDamageCharacter(avatars[k + 1].GetDamage() + room.GetRound() * 2, false);
						avatars[k + 1].ToDamageCharacter(avatars[k].GetDamage() + room.GetRound() * 2, false);
	...
	// 시간이 다 지났는데도 안끝난 전투가 있으면 강제 데미지 10
	{
		...
		for (int k = 0; k < avatarCount; k += 2)
		{
			...
			avatars[k].ToDamageCharacter(10 + room.GetRound() * 2, true);
			avatars[k + 1].ToDamageCharacter(10 + room.GetRound() * 2, true);
		...
		{
			{
				...
				for (int k = 0; k < avatarCount; k += 2)
				{
					...
					{
						playerAvatar.ToDamageCharacter(creepAvatar.GetDamage() + room.GetRound() * 2, false);
						creepAvatar.ToDamageCharacter(playerAvatar.GetDamage() + room.GetRound() * 2, false);
			...
			for (k = 0; k < avatarCount; k += 2)
			{
				...
				Log("BattleInfo", "[크립] {0}번 크립 몬스터 maxHp:{1}, hp:{2}, isFinish:{3}, mIsGhost:{4}", avatar2.GetNetworkID(), avatar2.GetMaxHP(), avatar2.GetHP(), avatar2.IsFinish(), avatar2.IsGhost());
			}
			Log("BattleInfo", "----------");
