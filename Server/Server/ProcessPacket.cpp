...
void ProcessPacket(int userID, char* buf)
{
	...
	case PacketType::cs_sc_addNewItem:
	{
		...
		Log("(cs_sc_addNewItem) {0} client add {1} item", pPacket->networkID, pPacket->itemCode);
	...
	case PacketType::cs_sc_changeCharacter:
	{
		...
		Log("(cs_sc_changeCharacter) {0} client change character {1}", pPacket->networkID, static_cast<int>(pPacket->characterType));
	...
	case PacketType::cs_sc_changeItemSlot:
	{
		...
		Log("{0} client change slot {1} <-> {2}", pPacket->networkID, pPacket->slot1, pPacket->slot2);
	...
	case PacketType::cs_battleReady:
	{
		...
		Log("{0} client send ready battle packet", pPacket->networkID);
	}
	break;
	default:
